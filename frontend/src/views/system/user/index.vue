...
onMounted(() => {
  ...
  // 注册「意图页面上下文」：意图面板打开时按本页可见数据取当前用户，
  // 于是「用户全景画像」「停用账号」这类意图点开即用，不需要用户再填一遍编号。
  // 返回值是注销函数，必须在卸载时调用，否则离开页面后上下文会残留。
  unregisterIntentContext = registerIntentPageContext('system/user', () => {
    const selected = ids.value?.[0];
    return {
      // 入参 Schema 声明为 string，这里显式转换，避免 JSON Schema 校验不过
      userId: selected === undefined || selected === null ? undefined : String(selected)
    };
  });
});
